...
class Tomarket:
    ...
    def process_queries(self, lines_per_file=10):
        if not os.path.exists('JOO.txt'):
            raise FileNotFoundError(f"File 'JOO.txt' Not Found. Please Ensure It Exists")

        with open('JOO.txt', 'r') as f:
            JOO = [line.strip() for line in f if line.strip()]

        if not JOO:
            raise ValueError("File 'JOO.txt' Is Empty")
        ...
        for i in range(0, len(JOO), lines_per_file):
            chunk = JOO[i:i + lines_per_file]
    ...
    def main(self, accounts):
        while True:
            try:
                ...
                total_balance = 0
                d = "YOUSEF"
                ONYS = render(f'{d}', colors=['red', 'yellow'], align='center')
                print(ONYS)
                print( "\033[0;92m My Channel : https://t.me/kingelnet" )
...
if __name__ == '__main__':
    try:
        ...
        account_files.sort(key=lambda x: int(re.findall(r'\d+', x)[0]))
        f = "welcome"
        JONYS = render(f'{f}', colors=['green', 'yellow'], align='center')
        print(JONYS)
